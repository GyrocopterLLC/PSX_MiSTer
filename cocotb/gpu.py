...
def setDrawTPage_T(dfe, dtd, tpage):
    return (0xE1000000) | \
            (tpage) | \
            (((dtd) & 1) << 9) | \
            (((dfe) & 1) << 10)

async def setDithering(dut, dithering):
    texpage = getTPage(0, 0, 0 ,0)
    e = DR_TPAGE_T()
    #setDrawTPage(&e, 1, dithering, texpage)
    e.code = setDrawTPage_T(1, dithering, texpage)

    await gpu_send_gp0(dut, e.code)